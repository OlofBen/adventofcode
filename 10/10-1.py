...
def vissit(matrix, row, col, vissited : set): 
    ...
    for n_row, n_col in neightbours(row, col): 
        if is_inside(matrix, n_row, n_col) and matrix[n_row][n_col] == current_hight + 1 and (n_row, n_col) not in vissited: 
            sum += vissit(matrix, n_row, n_col, vissited)
...
for row, list in enumerate(input): 
    for col, value in enumerate(list): 
        if value == 0: 
            trailheads_sum += vissit(input, row, col, set())
